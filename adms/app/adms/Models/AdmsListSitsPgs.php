...
class AdmsListSitsPgs
{
    ...
    public function listSitsPgs(int $page = null):void
    {
        ...
        $pagination = new \App\adms\Models\helper\AdmsPagination(URLADM.'list-sits-pgs/index');
        ...
        $listSitUsers->fullRead("SELECT sits.id, sits.name AS sitsname, col.name AS colname, col.color FROM adms_sits_pgs AS sits INNER JOIN adms_colors AS col ON col.id=sits.adms_color_id ORDER BY sits.id DESC LIMIT :limit OFFSET :offset", "limit={$this->limitResult}&offset={$pagination->getOffset()}");
        ...
        if($this->resultBd){
            ...
            $_SESSION['msg'] = "<p class='alert alert-warning'>Erro! Nenhuma Situação de pagina encontrada!</p>";
            $this->result - false;
        }
    }
}