...
class EditPageMenu
{
    ...
    public function index(int|string|null $id_level_page = null):void
    {
        ...
        if((!empty($this->id_level_page)) and (!empty($this->level)) and (!empty($this->pag)) and (empty($this->dataForm['SendEditPageMenu']))){
            $viewPageMenu = new \Adms\Models\AdmsEditPageMenu();
            $viewPageMenu->viewPageMenu($this->id_level_page);
    ...
    private function viewEditPageMenu():void
    {
        ...
        $listSelect = new \Adms\Models\AdmsEditPageMenu();
        ...
        $loadView = new \AdmsSrc\ConfigViewAdms("adms/Views/permission/editPageMenu", $this->data);
    ...
    private function editPageMenu():void
    {
        if(!empty($this->dataForm['SendEditPageMenu'])){
            ...
            $editPageMenu = new \Adms\Models\AdmsEditPageMenu();
            ...
            $_SESSION['msg'] = "<p class='alert alert-danger'>Erro 143! Item de Menu não encontrado!</p>";
