    ...
    <?php if (!empty($this->data['viewPage'])) {
        ...
        <div class="col-12 text-center p-4">
            ...
            <?php if($this->data['button']['edit_page']) { ?>
                <a class="btn btn-sm btn-outline-warning mx-1" href="<?= URLADM; ?>edit-page/index/<?= $id; ?>"><i class='fa-solid fa-pen-to-square'></i> Editar</a> <?php } ?>
            <?php if($this->data['button']['delete_page']) { ?>
            <a class="btn btn-sm btn-outline-danger mx-1" href="<?= URLADM; ?>delete-page/index/<?= $id; ?>" onclick="return confirm('Tem certeza que deseja excluir o registro?')"><i class='fa-solid fa-trash-can'></i> Apagar</a><?php } ?>
        </div>
        <?php } ?>
    </div>
</div>
