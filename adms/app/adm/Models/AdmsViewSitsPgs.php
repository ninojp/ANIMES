...
class AdmsViewSitsPgs
{
    ...
    public function viewSitsPgs(int $id): void
    {
        ...
        $viewSitPg = new \App\adms\Models\helper\AdmsRead();
        //usa o objeto para instânciar o método:fullRead(), passando a query desejada
        $viewSitPg->fullRead("SELECT sits.id, sits.name AS sitsname, sits.created, sits.modified, col.name AS colname, col.color FROM adms_sits_pgs AS sits INNER JOIN adms_colors AS col ON col.id=sits.adms_color_id WHERE sits.id=:id LIMIT :limit", "id={$this->id}&limit=1");
        ...
        if ($this->resultBd) {
            ...
            $_SESSION['msg'] = "<p class='alert alert-warning'>Erro! Situação da Pagina não encontrada!</p>";
            $this->result = false;
        }
    }
}
