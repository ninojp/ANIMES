...
class AdmsListUsers
{
    ...
    public function searchUserName(): void
    {
        ...
        $pagination->pagination("SELECT COUNT(usr.id) AS num_result FROM adms_users AS usr
        INNER JOIN adms_access_levels AS lev ON lev.id=usr.access_level_id 
        WHERE (lev.order_levels >:order_levels) AND (usr.name LIKE :search_name)",
        ...
        //INNER JOIN, é obrigátorio(para retornar o registro) q a chave EXTRANGEIRA:adms_sits_user_id exista na tabela outra tabela, a qual está se fazendo o inner join(adms_sits_users)
        $listUsersName->fullRead("SELECT usr.id, usr.name AS name_usr, usr.email, usr.adms_sits_user_id, sit.name AS name_sit, col.name AS name_col, col.color
        FROM adms_users AS usr
        INNER JOIN adms_access_levels AS lev ON lev.id=usr.access_level_id
        INNER JOIN adms_sits_users AS sit ON sit.id=usr.adms_sits_user_id 
        INNER JOIN adms_colors AS col ON sit.adms_color_id=col.id 
        WHERE (lev.order_levels >:order_levels) AND (usr.name LIKE :search_name)
        ORDER BY usr.id DESC LIMIT :limit OFFSET :offset", "order_levels=".$_SESSION['order_levels']."&search_name={$this->searchNameValue}&limit={$this->limitResult}&offset={$pagination->getOffset()}");
        ...
        if ($this->resultBd) {
            ...
            $_SESSION['msg'] = "<p class='alert alert-warning'>Erro (searchUserName())! Nenhum usuário encontrado!</p>";
            $this->result = false;
        }
    }
    /** ============================================================================================
    ...
    public function searchUserEmail(): void
    {
        ...
        $pagination->pagination("SELECT COUNT(usr.id) AS num_result FROM adms_users AS usr
        INNER JOIN adms_access_levels AS lev ON lev.id=usr.access_level_id
        WHERE (lev.order_levels >:order_levels) AND (usr.email LIKE :search_email)",
        ...
        //INNER JOIN, é obrigátorio(para retornar o registro) q a chave EXTRANGEIRA:adms_sits_user_id exista na tabela outra tabela, a qual está se fazendo o inner join(adms_sits_users)
        $listUsersEmail->fullRead("SELECT usr.id, usr.name AS name_usr, usr.email, usr.adms_sits_user_id, sit.name AS name_sit, col.name AS name_col, col.color
        FROM adms_users AS usr
        INNER JOIN adms_access_levels AS lev ON lev.id=usr.access_level_id
        INNER JOIN adms_sits_users AS sit ON sit.id=usr.adms_sits_user_id
        INNER JOIN adms_colors AS col ON sit.adms_color_id=col.id
        WHERE (lev.order_levels >:order_levels) AND (usr.email LIKE :search_email)
        ORDER BY usr.id DESC LIMIT :limit OFFSET :offset",
        "order_levels=".$_SESSION['order_levels']."&search_email={$this->searchEmailValue}&limit={$this->limitResult}&offset={$pagination->getOffset()}");
        ...
        if ($this->resultBd) {
            ...
            $_SESSION['msg'] = "<p class='alert alert-warning'>Erro (searchUserEmail())! Nenhum e-mail encontrado!</p>";
            $this->result = false;
        }
    }
}
